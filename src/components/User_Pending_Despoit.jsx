...
const User_Pending_Deposit = () => {
  ...
  const user_pending_deposit = useSelector((store)=>store.user.userPendingDeposit);
  ...
  return (
    ...
    <h2 className="text-3xl font-semibold mb-6 text-center">User Pending Deposit</h2>
    ...
    {user_pending_deposit && user_pending_deposit.length > 0 ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {user_pending_deposit.map((machine) => (
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105" key={machine.machine_id}>
            <img
              src={machine.url}
            ...
            <p className="text-sm text-gray-400 mb-3">Valid for <span className="text-yellow-300">{machine.valid_days}</span> days</p>
            <button
      ...
      <p className="text-gray-400 text-center">No pending deposits available.</p>
    )}
  </div>
